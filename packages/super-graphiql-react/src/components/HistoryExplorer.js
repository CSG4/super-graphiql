  ...
  const newQueries =
    nextProps.runID === currProps.runID
      ...
      : executedQueries.filter(
          nextQuery =>
            !lastQuerySaved.some(
              pastQuery => pastQuery.query === nextQuery.query
            )
        );
...
export class HistoryExplorer extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    let historyQueries = this.historyStore.fetchAll() || [];
    let pinnedQueries = this.pinnedStore.fetchAll();
  ...
  handleSearch = searchParams => {
    const match = this.historyStore.items.filter(entry =>
